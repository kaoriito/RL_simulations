...
while trial < totalTrials
    ...
    if s1Choice == 1
        transition=transitionMat(countS1_1,1);
        ...
        transition=transitionMat(countS1_2,2);
...
%     end
    ...
    dataSheet(trial,10)=Qmb(1);
    dataSheet(trial,11)=Qmb(2);
    ...
    softmax1=exp(beta*Qmb(1))/(exp(beta*Qmb(1))+exp(beta*Qmb(2)));
    softmax2=exp(beta*Qmb(2))/(exp(beta*Qmb(1))+exp(beta*Qmb(2)));

%     softmax1=1/(1+exp(-beta*(Qmb(1)-Qmb(2))));
%     softmax2=1/(1+exp(-beta*(Qmb(2)-Qmb(1))));
...
% 
%     softmax1=exp(beta*(Qmb(1)+p*rep_a))/(exp(beta*(Qmb(1)+p*rep_a))+exp(beta*(Qmb(2)+p*rep_a)));
%     softmax2=exp(beta*(Qmb(2)+p*rep_a))/(exp(beta*(Qmb(1)+p*rep_a))+exp(beta*(Qmb(2)+p*rep_a)));
...
% legend('Qmb(1)','softmax1','Qmb(2)','softmax2','FontSize',14)
% 
% figure
% plot(dataSheet(:,1),dataSheet(:,10),...
%     dataSheet(:,1),dataSheet(:,14))
% legend('Qmb(1)','softmax1','FontSize',14)
% 
% 
% figure
% plot(dataSheet(:,1),dataSheet(:,11),...
%     dataSheet(:,1),dataSheet(:,15))
% legend('Qmb(2)','softmax2','FontSize',14)